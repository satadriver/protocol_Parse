...
pcap_t * Winpcap::init(string devname, int delay, unsigned long netmask) {
	...
	ret = pcap_compile(pcapt, &stBpfp, PCAP_PORT_FILTER, TRUE, netmask);
	if (ret < 0)
	{
...
int Winpcap::dumpfile( string filename,string devname) {
...
*/
	pcap_t* pcaptsend = pcap_open_live(pcapdevname.c_str(), WINPCAP_MAX_PACKET_SIZE, PCAP_OPENFLAG_PROMISCUOUS, 
		1000, ebuf);
	if (pcaptsend == NULL)
	{
